...
def set_status():
    if pause == True:
        canvas.itemconfig(text_id, text = 'Пауза')
    else:
        canvas.itemconfig(text_id, text = 'Вперёд')
...
def check_finish():
    global game_over
    coords_1 = canvas.coords(player1)
    coords_2 = canvas.coords(player2)
    coords_f = canvas.coords(finish_id)
    x1_right = coords_1[2]
    x2_right = coords_2[2]
    x_finish = coords_f[0]
    if x1_right >= x_finish:
        canvas.itemconfig(text_id, text='Победа верхнего игрока')
        print('Победа верхнего игрока')
        game_over = True
    if x2_right >= x_finish:
        canvas.itemconfig(text_id, text='Победа нижнего игрока')
        print('Победа нижнего игрока')
        game_over = True
